...
const API_BASE_URL = baseUrl + "/Trainer/HomeWork";
const API_BASE_URL_STUDENTHOMEWORK = baseUrl + "/Trainer/StudentHomework";
...
//Trainer'ın atadığı ödevleri getir
export async function getHomeworkAssignedByTrainer() {
  console.log(savedData[2]);
  try {
    const response = await axios.get(
      `${API_BASE_URL}/GetHomeworkAssignedByTrainer?id=${savedData[2]}`,
      {
        headers: {
          Authorization: `Bearer ${savedData[0]}`,
        },
...
//Öğrencilere atanan tüm ödevleri getirir.
