...
const API_BASE_URL_SUPLEMENTARYTAGS =
  baseUrl + "/Admin/SupplementaryResourceTag";
const API_BASE_URL_SUPLEMENTARYEDUCATIONSUBJECTS =
  baseUrl + "/Admin/SupplementaryResourceEducationSubject";
...
export async function getDeletedResourceById(id) {
  const validationErrors = {};
  try {
    const response = await axios.get(
      `${API_BASE_URL_SUPLEMENTARY}/DeletedResource?id=${id}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${savedData[0]}`,
...
export async function recoverResource(resourceData) {
  const validationErrors = {};
  try {
    const response = await axios.put(
      `${API_BASE_URL_SUPLEMENTARY}/Recover`,
      resourceData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
...
export async function permanentlyDeleteSoftDeletedResource(id) {
  try {
    const response = await axios.delete(
      `${API_BASE_URL_SUPLEMENTARY}/PermanentlyDeleteDeledResource/?id=${id}`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${savedData[0]}`,
    ...
    const errorMessage = error.response?.data?.message || "Bir hata oluştu.";
    console.error(errorMessage);
    // Hata durumunda gerekli işlemleri yapabilirsiniz, örneğin bir hata iletişi gösterebilirsiniz.
    throw error;
  }
}
